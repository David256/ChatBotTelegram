...
!replace, !unlearn, !purge, !version, !words, !limit, !alias, !save, !censor, !uncensor, !owner"
                    ...
                    for filename, data in [
                        ...
                        with AtomicFile(filename, 'w') as f:
                            ...
                            for key in data.keys():
                                wordlist.append([key, len(str(data[key]))])
                            wordlist.sort(lambda x, y: cmp(x[1], y[1]))
                            #map((lambda x: f.write(str(x[0]) + "\n\r")), wordlist)
                            [ f.write(str(x[0].encode('utf-8')) + "\n\r") for x in wordlist]
    ...
    def reply(self, body):
        ...
        while done == 0:
            ...
            word = str(sentence[0].encode('utf-8').split(" ")[0])
            word = word.decode('utf-8')
            for x in xrange(0, len(self.words[word]) - 1): # DUDO de ese decode
        ...
        while done == 0:
            #create a dictionary wich will contain all the words we can found before the "chosen" word
            post_words = {"" : 0}
            word = str(sentence[-1].encode('utf-8').split(" ")[-1])
            for x in self.words[word.decode('utf-8')]: # DUDO de ese decode
                if len(x) == 10:
                    l, w = struct.unpack("lH", x)
                else:
                    l, w = struct.unpack("iH", x)
    ...
    def learn(self, body, num_context = 1):
        ...
        def learn_line(self, body, num_context):
            ...
            voyelles = "áéíóúÁÉÍÓÚaeiouAEIOUäëïöüÄËÏÖÜâêîôûÂÊÎÔÛÀÈÌÒÙàèìòù"
            for x in xrange(0, len(words)):
